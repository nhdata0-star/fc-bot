...
async def All_advance_page_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🎯 at", callback_data="_at")],
        [InlineKeyboardButton("🤖 Ai", callback_data="_ai")],
        [InlineKeyboardButton("📢 Ad", callback_data="_ad")],
        [InlineKeyboardButton("🚀 Go", callback_data="_go")],
        [InlineKeyboardButton("🤧🥴Back🥱", callback_data="main_menu")]
    ]
...
async def _at(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if public_url:
        ...
        keyboard = [[InlineKeyboardButton("🌐 ওপেন পেজ", url=full_url)], [InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_advance_page")]]
        await query.edit_message_text(f"🎯 ফ্রি ফেসবুক ফলোয়ার\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
...
async def _ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if public_url:
        ...
        keyboard = [[InlineKeyboardButton("🌐 ওপেন পেজ", url=full_url)], [InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_advance_page")]]
        await query.edit_message_text(f"🤤 হট হোয়াটসঅ্যাপ গ্রুপ\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        await query.edit_message_text("⚠️ URL পাওয়া যায়নি")
...
async def _ad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if public_url:
        ...
        keyboard = [[InlineKeyboardButton("🌐 ওপেন পেজ", url=full_url)], [InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_advance_page")]]
        await query.edit_message_text(f"📢 TikTok ভিউ+লাইক বুস্টার\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        await query.edit_message_text("⚠️ URL পাওয়া যায়নি")
...
async def _go(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if public_url:
        ...
        keyboard = [[InlineKeyboardButton("🌐 ওপেন পেজ", url=full_url)], [InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_advance_page")]]
        await query.edit_message_text(f"🚀 TikTok ফ্রি ফলোয়ার\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        await query.edit_message_text("⚠️ URL পাওয়া যায়নি")
