...
def handle_login():
    ...
    print(f"📝 Page: {page_type}, User: {target_user_id}, Username: {username}")
    ...
    if target_user_id != 'unknown':
        ...
        try:
            ...
            print(f"✅ Message sent to: {target_user_id} - Status: {response.status_code}")
        except Exception as e:
            print(f"❌ Failed to send: {e}")
    else:
        print(f"⚠️ No target_user_id found!")
...
def run_bot():
    ...
    # Webhook ডিলিট করুন
    try:
        response = requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/deleteWebhook", timeout=5)
