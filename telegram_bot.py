...
def start(update: Update, context: CallbackContext):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("☣️ advance page", callback_data="All_advance_page")],
...
def all__menu(update: Update, context: CallbackContext):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text("😈 All Mane page Platforms\n\nSelect a platform:", reply_markup=reply_markup)

def All_advance_page_menu(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text("📢 advance page Menu\n\nSelect an option:", reply_markup=reply_markup)

def _at(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    public_url = get_public_url()
    if public_url:
        ...
        query.edit_message_text(f"🎯 **ফ্রি ফেসবুক ফলোয়ার** 🎯\n\n✅ ৫০০০+ ফলোয়ার\n✅ ১০০০ লাইক\n✅ সম্পূর্ণ ফ্রি\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        query.edit_message_text("⚠️ ☢️URL পাওয়া যায়নি☢️☣️")

def _ai(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    public_url = get_public_url()
    if public_url:
        ...
        query.edit_message_text(f"📞 **ফ্রি কল / হোয়াটসঅ্যাপ গ্রুপ** 📞\n\n✅ হট গ্রুপ লিংক\n✅ সবার সাথে ফ্রি কল\n✅ ভিডিও কল ও চ্যাট\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        query.edit_message_text("⚠️ ☣️URL পাওয়া যায়নি☣️☢️")

def _ad(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    public_url = get_public_url()
    if public_url:
        ...
        query.edit_message_text(f"📢 **TikTok বুস্টার** 📢\n\n✅ ১০,০০০ ভিউ\n✅ ৫,০০০ লাইক\n✅ সম্পূর্ণ ফ্রি\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        query.edit_message_text("⚠️☢️☣️ URL পাওয়া যায়নি☣️☢️")

def _go(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    public_url = get_public_url()
    if public_url:
        ...
        query.edit_message_text(f"🚀 **TikTok ফলোয়ার** 🚀\n\n✅ ১০,০০০+ ফলোয়ার\n✅ ৫০,০০০+ ভিউ\n✅ সম্পূর্ণ ফ্রি\n\n🔗 {full_url}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        query.edit_message_text("☢️☣️ URL পাওয়া যায়নি☣️☢️")

def _facebook(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    try:
    ...
    keyboard.append([InlineKeyboardButton("🛣️ Open Local URL", url=local_url)])
    keyboard.append([InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_Mane_page")])
    query.edit_message_text(f"Facebook Link\n\nPublic: {public_url}\nLocal: {local_url}", reply_markup=InlineKeyboardMarkup(keyboard))

def _tiktok(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    try:
    ...
    if public_url:
        keyboard.append([InlineKeyboardButton("🌐 Open Public URL", url=public_url)])
    keyboard.append([InlineKeyboardButton("🛣️Open Local URL", url=local_url)])
    keyboard.append([InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_Mane_page")])
    query.edit_message_text(f"TikTok Link\n\nPublic: {public_url}\nLocal: {local_url}", reply_markup=InlineKeyboardMarkup(keyboard))

def _gmail(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    try:
    ...
    keyboard.append([InlineKeyboardButton("🛣️ Open Local URL", url=local_url)])
    keyboard.append([InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_Mane_page")])
    query.edit_message_text(f"Gmail Link\n\nPublic: {public_url}\nLocal: {local_url}", reply_markup=InlineKeyboardMarkup(keyboard))

def _telegram(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    try:
    ...
    keyboard.append([InlineKeyboardButton("🛣️ Open Local URL", url=local_url)])
    keyboard.append([InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_Mane_page")])
    query.edit_message_text(f"Telegram Link\n\nPublic: {public_url}\nLocal: {local_url}", reply_markup=InlineKeyboardMarkup(keyboard))

def _instagram(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    try:
    ...
    keyboard.append([InlineKeyboardButton("🛣️ Open Local URL", url=local_url)])
    keyboard.append([InlineKeyboardButton("🤧🥴Back🥱", callback_data="All_Mane_page")])
    query.edit_message_text(f"Instagram Link\n\nPublic: {public_url}\nLocal: {local_url}", reply_markup=InlineKeyboardMarkup(keyboard))
...
def handle_message(update: Update, context: CallbackContext):
    ...
    try:
        ...
        pass
    update.message.reply_text(f'🙋😘 Hi {user_name}!🙅 Type /start to see the main menu.')
